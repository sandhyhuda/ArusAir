...
# Tambahkan matplotlib setting untuk mencegah warning tentang rcParams
...
def tampilkan_penjelasan():
    ...
    with col1:
        try:
            ...
            st.write("Alternatif visualisasi: Penampang saluran air berbentuk melengkung dengan aliran dari kiri ke kanan.")
    
    with col2:
        try:
            # Create demonstration data for flow rate chart
            times = np.array([0, 10, 20, 30, 40, 50, 60])
            volumes = np.array([0, 50, 100, 150, 200, 250, 300])
            ...
            chart = alt.Chart(data).mark_line(point=True).encode(
                ...
                y=alt.Y('Volume (Liter):Q', title='Volume (Liter)')
            ).properties(
            ...
            st.error(f"Error menampilkan grafik: {e}")
            st.write("Hubungan Volume dan Waktu: Grafik menunjukkan peningkatan linear volume terhadap waktu.")
            st.write("Debit Air = 5 L/s")
...
def tampilkan_kalkulator():
    ...
    with st.form("form_debit"):
        if metode == "Volume/Waktu":
            if "m³/s" in satuan_debit:
                volume = st.number_input("Volume Air (m³):", min_value=0.0, value=0.0, step=0.1)
                satuan_volume = "m³"
            else:
                volume = st.number_input("Volume Air (L):", min_value=0.0, value=0.0, step=0.1)
                satuan_volume = "L"
            
            waktu = st.number_input("Waktu (detik):", min_value=0.1, value=1.0, step=0.1)
            
            # Nilai default untuk luas penampang dan kecepatan untuk perhitungan
            luas_penampang = 0.0
            kecepatan = 0.0
            
        else:  # Luas Penampang × Kecepatan
            luas_penampang = st.number_input("Luas Penampang (m²):", min_value=0.0, value=0.0, step=0.01)
            kecepatan = st.number_input("Kecepatan Aliran (m/s):", min_value=0.0, value=0.0, step=0.01)
            
            # Nilai default untuk volume dan waktu untuk perhitungan
            volume = 0.0
            waktu = 1.0
            satuan_volume = "m³" if "m³/s" in satuan_debit else "L"
    ...
    if submit_button:
        try:
            ...
            st.error("Error: Waktu tidak boleh nol. Silakan masukkan nilai waktu yang valid.")
        except Exception as e:
...
def progress_bar(value, unit):
    try:
        ...
        # Menyesuaikan nilai untuk visualisasi (memastikan nilai tidak negatif)
        norm_value = max(0, min(value, max_value)) / max_value
        ...
        st.error(f"Error menampilkan visualisasi: {str(e)}")
        st.write(f"Nilai debit: {value} {unit}")
...
def main():
    try:
        ...
        st.write("Silakan refresh halaman atau hubungi administrator jika masalah berlanjut.")

if __name__ == '__main__':
    try:
        main()
    except Exception as e:
        st.error(f"Aplikasi tidak dapat dimuat: {str(e)}")
        st.write("Silakan periksa instalasi Streamlit dan library pendukung.")
